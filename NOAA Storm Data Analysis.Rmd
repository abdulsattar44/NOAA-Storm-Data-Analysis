---
title: "NOAA Strom Data Analysis"
output: html_document
date: "2023-04-18"
---
## Analysis of U.S. Storms Events and their Impact on Population, Health and the Economy
...
stormDataFile <- "data/repdata_data_StormData.csv"
if(!file.exists("data")){
  dir.create('data')
}
...
When processing large dataset, compute performance can be improved by taking a subset of variables required for analysis. For this analysis only necessary variables will be included in the dataset, which are following
...
combining similar Event Type values into unique categories. For example, if the EVTYPE column contains entries like "WILDFIRE", "STRUCTURE FIRE", and "BUSH FIRE", they will all be replaced with "FIRE".
...
stormDataTidy$EVTYPE <- gsub('.*SNOW.*', 'WINTER', stormDataTidy$EVTYPE)# AVALANCHE
stormDataTidy$EVTYPE <- gsub('.*AVALANCE.*', 'AVALANCHE', stormDataTidy$EVTYPE)
# BLIZZARD
stormDataTidy$EVTYPE <- gsub('.*BLIZZARD.*', 'BLIZZARD', stormDataTidy$EVTYPE)
# CLOUD
...
```{r health-impact-table, echo = TRUE, results = 'asis'}
